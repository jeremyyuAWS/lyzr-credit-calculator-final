...
interface SetupCostsTabProps {
  accountId: string;
}

export default function SetupCostsTab({ accountId }: SetupCostsTabProps) {
  ...
  async function loadSettings() {
    ...
    try {
      ...
      console.error('Error loading setup costs:', error);
    } finally {
      setLoading(false);
    }
  }
  ...
  if (loading) {
    return (
      <div className="p-6 ">
        <div className="text-center py-8 text-gray-600">Loading setup costs...</div>
      </div>
    );
  }
  ...
  return (
    <div className="p-6 ">
      <p className="text-sm text-gray-600 mb-6">
        Setup costs are one-time charges incurred when creating agents, tools, knowledge bases, and other resources. These are separate from ongoing runtime costs.
      </p>

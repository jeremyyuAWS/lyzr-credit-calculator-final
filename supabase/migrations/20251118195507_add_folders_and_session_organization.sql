/*
  ...
  Adds folder management capabilities for organizing chat sessions.
  
  1. New Tables
    - `chat_folders` - Folder management
  
  2. Changes to Existing Tables
    - `chat_sessions` - Add folder_id and sort_order
*/
...
CREATE POLICY "Public can read chat folders"
  ON chat_folders FOR SELECT TO public USING (true);
CREATE POLICY "Public can insert chat folders"
  ON chat_folders FOR INSERT TO public WITH CHECK (true);
CREATE POLICY "Public can update chat folders"
  ON chat_folders FOR UPDATE TO public USING (true) WITH CHECK (true);
CREATE POLICY "Public can delete chat folders"
  ON chat_folders FOR DELETE TO public USING (true);

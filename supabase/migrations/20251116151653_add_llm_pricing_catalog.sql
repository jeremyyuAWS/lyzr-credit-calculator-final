/*
  ...
  1. New Tables
    - `llm_pricing`
      ...
      - `input_cost_per_million` (numeric) - Input token cost per 1M tokens in credits
      - `output_cost_per_million` (numeric) - Output token cost per 1M tokens in credits
      - `comment` (text) - Usage notes
    ...
    - `pricing_version_log`
      - `id` (uuid, primary key)
      - `version` (text) - Version identifier (e.g., 2025.11.16-01)
      - `updated_by` (text) - Always 'admin'
  ...
  3. Initial Data
    - Seed with comprehensive LLM pricing from spreadsheet
...
-- Seed LLM pricing data from spreadsheet
...
INSERT INTO pricing_version_log (version, updated_by, change_summary) VALUES
  ('2025.11.16-01', 'admin', 'Initial LLM pricing catalog with 21 models from DeepSeek, OpenAI, Anthropic, Gemini, and Groq')
